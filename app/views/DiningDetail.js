...
var DiningDetail = React.createClass({
				...
				<ScrollView contentContainerStyle={[css.scroll_main, css.whitebg]}>
					<View style={css.dl_container}>
						...
						<View style={css.dl_market_directions}>
							...
							<TouchableHighlight style={css.dl_dir_traveltype_container} underlayColor={'rgba(200,200,200,.1)'} onPress={ () => { general.gotoNavigationApp('walk', this.state.currentCoords.lat, this.state.currentCoords.lon) }}>
	...
	gotoDiningNutrition: function(data) {
		this.props.navigator.push({ id: 'DiningNutrition', name: data.name, title: data.name, menuItem: data, component: DiningNutrition });
	},
