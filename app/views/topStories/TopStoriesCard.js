	...
	refresh() {
		...
		.catch((error) => {
			...
			if (this.fetchErrorLimit > this.fetchErrorCounter) {
				...
				logger.error('ERR: fetchTopStories: refreshing again in ' + this.fetchErrorInterval/1000 + ' sec');
				this.refreshTimer = setTimeout( () => { this.refresh() }, this.fetchErrorInterval);
			} else {
				logger.error('ERR: fetchTopStories: Limit exceeded - max limit:' + this.fetchErrorLimit);
