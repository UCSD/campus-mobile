...
class EventDateTime extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    try {
      ...
      if (sameDay) {
        if (!unspecifiedTime) {
          return Text(startMonthDay + ', ' + startTime + ' - ' + endTime); // ex: Jan. 1, 8:00 AM - 12:00 PM
        } else {
  ...
  Widget build(BuildContext context) {
    try {
      ...
      if (sameDay) {
        ...
        var endYear = endMonthDayYear.substring(endMonthDayYear.indexOf(',') + 2, endMonthDayYear.length);
