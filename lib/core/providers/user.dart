...
class UserDataProvider extends ChangeNotifier {
  ...
  Future _loadSavedAuthenticationModel() async {
    ...
    if (authBox.get('AuthenticationModel') == null) {
      ...
      print("Attempting silent login after loading saved authentication model.");
      await silentLogin();
    }
  }
  ...
  /// Get encrypted password from device
  ...
  /// Encrypt credentials and store on device
  void _encryptAndSaveCredentials(String username, String password) {
    final pkString = dotenv.get('USER_CREDENTIALS_PUBLIC_KEY');
    final rsaParser = RSAKeyParser();
    final pc.RSAPublicKey publicKey = rsaParser.parse(pkString) as RSAPublicKey;
  ...
  /// Authenticate user with manual login
  Future manualLogin(String username, String password) async {
    print("manualLogin: Starting manual login for user: $username");
    _error = null;
    _isLoading = true;
    notifyListeners();
    ...
    if (username.isNotEmpty && password.isNotEmpty) {
      _encryptAndSaveCredentials(username, password);
      bool silentLoginSuccess = await silentLogin();
      if (silentLoginSuccess) {
        print("manualLogin: Silent login successful for user: $username");
        if (_userProfileModel.classifications!.student!) {
          cardsDataProvider.showAllStudentCards();
        } else if (_userProfileModel.classifications!.staff!) {
          cardsDataProvider.showAllStaffCards();
        ...
        print("manualLogin: Silent login failed. Error: ${_authenticationService.error}");
        _error = _authenticationService.error;
        _isLoading = false;
        notifyListeners();
        return false;
    ...
    print("manualLogin: Username or password not provided.");
    _error = 'Username or password not found';
    _isLoading = false;
    notifyListeners();
    return false;
  ...
  /// Logs user in silently with stored credentials
  Future<bool> silentLogin() async {
    print("silentLogin: Starting silent login.");
    _isInSilentLogin = true;
    notifyListeners();
    String? username = await getUsernameFromDevice();
    String? encryptedPassword = await _getEncryptedPasswordFromDevice();
    ...
    if (username != null && encryptedPassword != null) {
      print("silentLogin: Found stored credentials for user: $username");
      final String base64EncodedWithEncryptedPassword = base64.encode(utf8.encode(username + ':' + encryptedPassword));
      resetHomeScrollOffset();
      resetAllCardHeights();
      resetNotificationsScrollOffset();

      bool result = await _authenticationService.silentLogin(base64EncodedWithEncryptedPassword);
      if (result) {
        print("silentLogin: Authentication service returned success.");
        await updateAuthenticationModel(_authenticationService.data!);
        await fetchUserProfile();
        var _cardsDataProvider = CardsDataProvider();
        _cardsDataProvider.updateAvailableCards(_userProfileModel.ucsdaffiliation);
        ...
        print("silentLogin: Authentication service failed. Error: ${_authenticationService.error}");
      }
    } else {
      print("silentLogin: Missing stored credentials (username or encrypted password is null).");
    }
    logout();
    return false;
  }
  ...
  /// Toggle notifications for a topic
  void toggleNotifications(String topic) {
    if (_userProfileModel.subscribedTopics!.contains(topic))
      _userProfileModel.subscribedTopics!.remove(topic);
    else
  ...
  Future fetchUserProfile() async {
    print("fetchUserProfile: Attempting to fetch user profile.");
    _error = null;
    _isLoading = true;
    notifyListeners();
    ...
    if (isLoggedIn) {
      ...
      if (await _userProfileService.downloadUserProfile(headers)) {
        ...
        print("fetchUserProfile: Failed to download user profile. Error: ${_userProfileService.error}");
        _error = _userProfileService.error;
      }
    } else {
      print("fetchUserProfile: Not logged in.");
      _error = 'not logged in';
    }
    _isLoading = false;
    notifyListeners();
  ...
  Future<UserProfileModel> _createNewUser(UserProfileModel profile) async {
    await _pushNotificationDataProvider.fetchTopicsList();
    print("_createNewUser: Creating a new user profile.");
    try {
      profile.username = await getUsernameFromDevice();
      profile.ucsdaffiliation = _authenticationModel.ucsdaffiliation;
      profile.pid = _authenticationModel.pid;
      ...
      print("_createNewUser error: ${e.toString()}");
    }
    return profile;
  }

  /// Post the User Profile to the server
  Future postUserProfile(UserProfileModel profile) async {
    print("postUserProfile: Posting user profile.");
    _error = null;
    _isLoading = true;
    notifyListeners();
