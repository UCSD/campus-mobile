...
class ChatProvider extends ChangeNotifier {
  // MA-707 feedback service START
  ChatProvider(this._userDataProvider, {ChatFeedbackGateway? chatFeedbackGateway})
    : _chatFeedbackGateway = chatFeedbackGateway ?? ChatFeedbackService(_userDataProvider);
  // MA-707 feedback service END

  static const String _DEFAULT_SESSION_TITLE = 'New Chat';
  static const String _ASSISTANT_AUTHOR_ID = 'assistant';

  final UserDataProvider _userDataProvider;
  // MA-707 feedback gateway START
  final ChatFeedbackGateway _chatFeedbackGateway;
  // MA-707 feedback gateway END
  ...
  // MA-707 vote handling START
  Future<void> toggleMessageFeedback(String messageId, AssistantChatFeedback selectedFeedback) async {
    ...
    final int messageIndex = sessionMessages.indexWhere((AssistantChatMessage message) => message.id == messageId);
    if (messageIndex == -1) return;

    final AssistantChatMessage original = sessionMessages[messageIndex];
    final int? chatMessageId = original.feedbackMessageId;
    if (chatMessageId == null || original.isFeedbackPending) return;

    final bool removesFeedback = original.feedback == selectedFeedback;
    sessionMessages[messageIndex] = original.copyWith(
      feedback: removesFeedback ? null : selectedFeedback,
      clearFeedback: removesFeedback,
    ...
    try {
      if (removesFeedback) {
        await _chatFeedbackGateway.removeFeedback(chatSessionId: sessionId, chatMessageId: chatMessageId);
      } else {
        await _chatFeedbackGateway.createFeedback(
          chatMessageId: chatMessageId,
          isPositive: selectedFeedback == AssistantChatFeedback.upvote,
        );
      }

      sessionMessages[messageIndex] = sessionMessages[messageIndex].copyWith(isFeedbackPending: false);
      _setSessionMessages(sessionId, sessionMessages);
      await _persistSessionState(sessionId, saveAsActive: _activeSessionId == sessionId);
    } catch (_) {
      sessionMessages[messageIndex] = original;
      _setSessionMessages(sessionId, sessionMessages);
      _errorMessage = 'Feedback could not be saved. Please try again.';
    }

    notifyListeners();
  }
  // MA-707 vote handling END
