...
class AvailabilityStatus {
  AvailabilityStatus({required this.status, required this.data, required this.timestamp});
        ...
        if (returnList[index].subLocations.length > 3) {
          ...
          for (int i = 0; i < baseSubLocations.length; i += 3) {
            ...
            List<SubLocations> curSubList = baseSubLocations.sublist(
              i,
              i + 3 > baseSubLocations.length ? baseSubLocations.length : i + 3,
            );
...
class AvailabilityModel {
  AvailabilityModel({required this.id, required this.name, required this.subLocations});
...
class Floor {
  ...
  Map<String, dynamic> toJson() => {"id": id, "name": name, "percentage": percentage, "isActive": isActive};
}
