...
class FreeFoodService {
  ...
  Future<bool> fetchData(String id) async {
    ...
    try {
      ...
      if (e.toString().contains("401")) if (await NetworkHelper.getNewToken(headers)) return await fetchData(id);
      _error = e.toString();
      return false;
    } finally {
      _isLoading = false;
  ...
  Future<bool> fetchMaxCount(String id) async {
    ...
    try {
      ...
      /// if the authorized fetch failed we know we have to refresh the
      /// token for this service
      if (e.toString().contains("401")) if (await NetworkHelper.getNewToken(headers)) return await fetchMaxCount(id);
      _error = e.toString();
      return false;
    } finally {
      _isLoading = false;
  ...
  Future<bool> updateCount(String id, Map<String, dynamic> body) async {
    ...
    try {
      ...
      /// if the authorized fetch failed we know we have to refresh the
      /// token for this service
      if (e.toString().contains("401")) if (await NetworkHelper.getNewToken(headers)) return await updateCount(id, body);
      _error = e.toString();
      return false;
    } finally {
      _isLoading = false;
